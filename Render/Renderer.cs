...
namespace Wolf3DClone.Render
{
    public class Renderer
    {
            ...
            Texture2D enemyTex, Texture2D boltTex, List<Enemy> enemies, List<Projectile> playerBullets, GameTime gameTime)
        {
            ...
            // Стеля
            _sb.Draw(_pixel, new Rectangle(0, 0, sw, sh / 2), new Color(25, 25, 25));
            ...
            for (int x = 0; x < sw; x++)
            {
                ...
                _sb.Draw(tex, new Rectangle(x, (sh - h) / 2, 1, h), new Rectangle((int)(tc * tex.Width), 0, 1, tex.Height), new Color(Vector3.One * br));
            }

            // Спрайти (вороги, їхні кулі)
            ...
            _sb.Draw(_pixel, new Rectangle(barBG.X + 2, barBG.Y + 2, (int)((uiW - 4) * hRatio), uiH - 4), hCol); // Смужка
